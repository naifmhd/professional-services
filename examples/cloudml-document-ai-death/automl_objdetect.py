...
def detect_object(gcs_image_folder,
    ...
    table = utils.create_table(
        bq_client, bq_dataset_output, bq_table_output, schema)
    ...
    for blob in bucket.list_blobs(prefix=str(prefix + "/")):
        if blob.name.endswith(".png"):
            ...
            for result in request.payload:
                ...
                rows_to_insert = [
                    (str(blob.name).replace(".png", ".pdf").replace(prefix, "").replace("/", ""),
                     ...
                     result.image_object_detection.bounding_box.normalized_vertices[
                         0].x, result.image_object_detection.bounding_box.normalized_vertices[1].x,
                ...
                r_xmin = width * \
                    result.image_object_detection.bounding_box.normalized_vertices[0].x
                r_ymin = height * \
                    result.image_object_detection.bounding_box.normalized_vertices[0].y
                r_xmax = width * \
                    result.image_object_detection.bounding_box.normalized_vertices[1].x
                r_ymax = height * \
                    result.image_object_detection.bounding_box.normalized_vertices[1].y
                box = (r_xmin, r_ymin, r_xmax, r_ymax)
                im = Image.open(temp_local_filename)
                im2 = im.crop(box)
                im2.save(temp_local_filename.replace(
                    '.png', '-crop.png'), 'png')

                # Upload cropped image to gcs bucket
                new_file_name = os.path.join(gcs_cropped_image_folder, os.path.basename(
                    blob.name).replace('.png', '-crop.png'))
